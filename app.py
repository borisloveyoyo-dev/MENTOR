...
async def _apply_successful_payment(payment_id: str, user_id: int | None) -> None:
    async with async_session_maker() as session:
        ...
        if subscription is None:
            ...
            current_end = subscription.ends_at if subscription.ends_at and subscription.ends_at > now else now
...
async def yookassa_webhook(request: web.Request) -> web.Response:
    ...
    try:
        ...
        print(f"YooKassa webhook error: {exc}")
        return web.json_response(
            {"status": "error", "message": str(exc)},
            status=400,
        )
    except Exception as exc:
        print(f"YooKassa webhook unexpected error: {exc}")
        return web.json_response(
    ...
    try:
        if webhook_result.event == "payment.succeeded" and webhook_result.paid:
            ...
            async with async_session_maker() as session:
                payment_result = await session.execute(
                    select(Payment).where(Payment.yookassa_payment_id == webhook_result.payment_id)
                )
                payment = payment_result.scalar_one_or_none()
        ...
        print(f"YooKassa webhook DB update error: {exc}")
        return web.json_response(
...
async def on_startup() -> None:
    ...
    me = await bot.get_me()
    print(
        f"Telegram webhook установлен для @{me.username}: {TELEGRAM_WEBHOOK_URL}"
    )
